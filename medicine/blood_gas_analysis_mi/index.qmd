...
## 酸塩基平衡のマインドマップ(厳密には少し違うけど) {#acid-base-map}
...
flowchart TD
    ...
    AlbCorrectAGPosi --> AG["Anion Gap開大性代謝性アシドーシス"]
    AG --> HCO3["補正HCO3-を計算する"]
    HCO3 --> HCO3Acid["補正HCO3- \< 22"]
    HCO3 --> HCO3Alka["補正HCO3- \> 26"]

    HCO3Acid --> AGNAG["AG開大性代謝性アシドーシス+AG非開大性代謝性アシドーシス"]
    HCO3Alka --> AGAlka["AG開大性代謝性アシドーシス+代謝性アルカローシス"]


    AlbCorrectAGNega --> PrimaryEval["HCO3-/PaCO2でPrimaryの変化を評価"]
    PrimaryEval --> NAG["Anion Gap非開大性代謝性アシドーシス"]
    PrimaryEval --> MK["代謝性アルカローシス"]
    PrimaryEval --> RA["呼吸性アシドーシス"]
    PrimaryEval --> RK["呼吸性アルカローシス"]
...
flowchart TD
    ...
    MA --> AlbCorrectAG["Alb補正の上でのAG計算"]
    AlbCorrectAG --> NAG["Anion Gap非開大性代謝性アシドーシス"]
    AlbCorrectAG --> AG["Anion Gap開大性代謝性アシドーシス"]
    %% AG --> HCO3["補正HCO3-を計算する"]
    AG --> DKAetc["高度腎不全 <br> DKA <br> 乳酸Acidosis"]


    AG --> OsmGap["浸透圧ギャップを計算"]
    OsmGap --> OsmGapPosi["Osm Gap> 10-15 <br> 毒性アルコール中毒\(メタノール・エチレングリコールなど\)"]

    NAG --> UrineAG["尿中AGを計算"]
    UrineAG --> UrineAGPosi["尿中AG >0"]
    UrineAG --> UrineAGNega["尿中AG <<0"]
    UrineAGNega --> UrineAGNegadisease["下痢 <br> 回腸導管 <br> 胆膵管ドレナージ"]
    UrineAGPosi --> UrineOsmGap["尿中浸透圧ギャップ"]
    UrineOsmGap --> UrineOsmGapPosi["尿浸透圧ギャップ\>100 <br> トルエン中毒 <br> DKA回復期"]
    UrineOsmGap --> UrineOsmGapNega["尿浸透圧ギャップ\<\<100 <br> 保存期CKD <br> 尿細管Acidosis"]
