...
![](images/ordered_scale.png){height="50vh"}
...
![](images/num_publish_pro.png){height="70vh"}

- High impact journalではPROを含む研究が増えている！！ [@Eliya2021-yf]

## PROの特徴

![](images/scale_species.png)
...
![](images/pro_analysis_real2.png){height="30vh"}
...
test_df <- tibble::tibble(
    ...
    dplyr::mutate(
        y = log(x) * 40 + runif(n = 1, min = 5, max = 8)
    )
...
rms::survplot(rms::npsurv(Surv(time, status) ~ adhere, data = colon))
```
...
set.seed(42)
diff_plot_dat <- tibble::tibble(
    pre = rnorm(n = 300, mean = 75, sd = 20), 
    post = rnorm(n = 300, mean = 80, sd = 20)
) |> 
    dplyr::mutate(
        across(everything(), \(x)dplyr::if_else(x >= 100, 100, x)), 
        diff = post - pre, 
        diff = if_else(diff < -30, diff/2, diff)
    )

tinyplot::plt(diff ~ pre, data = diff_plot_dat, type = "lm", 
xlab = "Baseline PROM", ylab = "Difference of PROM", 
ylim = c(-30, 100)
)
tinyplot::tinyplot_add(type = "p", alpha = 0.6, col = "steelblue")
tinyplot::tinyplot_add(type = type_hline(h = 0), lty = 2)

bad_fit <- lm(post ~ pre, data = diff_plot_dat)

performance::check_model(bad_fit, check = c("qq", "normality", "linearity", "homogeneity"))
...
![](images/pro_visualize.jpg){height="50vh"}
...
![](images/pro_linechart_advanced.png){fig-align="center"}
...
![](images/pro_linechart_addci.png){height="50vh"}
...
tobit_df <- tibble::tibble(x = rnorm(100, mean = 55, sd = 16)) |> 
    dplyr::mutate(
        y = 1.6 * x + rnorm(n = 100, sd = 5), 
        y_ceil = dplyr::if_else(y >= 100, 100, y)
    )

lm_fit <- lm(y_ceil ~ x, data = tobit_df)

lm_txt <- paste0("y = ", round(lm_fit$coefficients['(Intercept)'], digits = 1), " + ", round(lm_fit$coefficients['x'], digits = 1), "x")

tobit_fit <- AER::tobit(y_ceil ~ x, left = 0, right = 100, data = tobit_df)
tobit_txt <- paste0("y = ", round(tobit_fit$coefficients['(Intercept)'], digits = 1), " + ", round(tobit_fit$coefficients['x'], digits = 1), "x")

ggplot() + 
    geom_point(data = dplyr::filter(tobit_df, y > 100), mapping = aes(x = x, y = y), alpha = 0.1, colour = "red") + 
    geom_point(data = tobit_df, mapping = aes(x = x, y = y_ceil), alpha = 0.5, colour = "steelblue") + 
    geom_smooth(data = tobit_df, method = lm, mapping = aes(x = x, y = y_ceil), 
    formula = y ~ x, colour = "steelblue", se = FALSE) + 
    geom_abline(intercept = tobit_fit$coefficients['(Intercept)'], slope = tobit_fit$coefficients['x'], colour = "red") + 
    geom_text(aes(x = 20, y = 60), label = lm_txt, colour = "steelblue") + 
    geom_text(aes(x = 28, y = 10), label = tobit_txt, colour = "red") + 
    xlim(c(0, 100)) + ylim(c(0, 160)) +
    theme_classic() 
...
ordinal_dat <- tibble::tribble(
    ...
    dplyr::mutate(
        class = forcats::fct_relevel(
            class, c("Very bad", "Bad", "Normal", "Good", "Very good")
        )
    )
...
ist3_visdat <- ist3_dat |>
    ...
    dplyr::mutate(
        ...
        delta_euroqol = euroqol18 - euroqol6,
        delta_cat = dplyr::case_when(
            delta_euroqol >= 10 ~ "Improve",
            10 > delta_euroqol & delta_euroqol > -10 ~ "No change",
            -10 > delta_euroqol ~ "Worse"
        ) |> forcats::fct_relevel(c("Improve", "No change", "Worse"))
    )
...
barplot_dat <- dplyr::select(ist3_visdat, treatment, delta_cat) |>
    tidyr::drop_na() |>
    dplyr::count(treatment, delta_cat) |>
    dplyr::mutate(total = sum(n), .by = treatment) |>
    dplyr::mutate(
        percent = n / total,
        percent_lab = scales::percent(percent, accuracy = .1)
    )

barplot <- ggpubr::ggbarplot(
    data = barplot_dat,
    x = "treatment", y = "percent", fill = "delta_cat",
    label = barplot_dat$percent_lab, lab.pos = "in"
)
...
lower <- cowplot::plot_grid(
    plotlist = list(lineplot, barplot), nrow = 1
)

plots <- cowplot::plot_grid(
    plotlist = list(upper, lower), nrow = 2
)
...
gtsummary::tbl_summary(
    ist3_dat_mod,
    by = missing_euroqol,
    include = c(age, sexm1, antiplat_rand, atrialfib_rand, sbprand, gcs_score_rand, sevendaycase, stroketype, ohs6),
    value = list(
        sevendaycase ~ "Event",
        atrialfib_rand ~ 1,
        antiplat_rand ~ 1
...
ggmice::plot_pattern(dplyr::select(ist3_dat_mod, -reverse_euroqol_comp, -missing_euroqol, -bad_euroqol_comp_bin))
```
...
miss_dat <- dplyr::select(
    ist3_dat_mod, -treatment_dbl, -reverse_euroqol_comp,
...
tobit_fit <- tobit(
    ohs_formula,
    left = 0, right = 6, data = ist3_dat_mod
)
...
tobit_fit_euroqol <- AER::tobit(
    euroqol_formula,
    left = 0, right = 100, data = ist3_dat_mod
)
...
parameters::compare_parameters(
    ...
    keep = "treatment_dbl",
    select = "{estimate} [{ci_low} - {ci_high}], {p}"
) |>
    parameters::display(format = "html")
```
...
tobit_fit_euroqol2 <- AER::tobit(
    euroqol_formula2,
    left = 0, right = 101, data = ist3_dat_mod
)
