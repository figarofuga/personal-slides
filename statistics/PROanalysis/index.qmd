...
ist3_dat_mod <- ist3_dat |>
    ...
    dplyr::select(itt_treat, age, sexm1, indepinadl_rand, antiplat_rand, atrialfib_rand, sbprand, dbprand, gcs_score_rand, nihss, sevendaycase, stroketype, R_infarct_size, ohs6, dead6mo, euroqol6, reverse_euroqol_comp, bad_euroqol_comp_bin, missing_euroqol)
...
gtsummary::tbl_summary(
    ...
    include = -c(euroqol6, dead6mo, reverse_euroqol_comp, bad_euroqol_comp_bin),
    value = list(
        sevendaycase ~ "Event",
        indepinadl_rand ~ 1,
        atrialfib_rand ~ 1,
...
naniar::vis_miss(ist3_dat_mod)

naniar::gg_miss_fct(x = ist3_dat_mod, fct = stroketype)
...
ohs_formula <- as.formula(ohs6 ~ itt_treat + age + sexm1 + indepinadl_rand + antiplat_rand + atrialfib_rand + sbprand + dbprand + gcs_score_rand + nihss + sevendaycase + stroketype + R_infarct_size)
...
euroqol_formula <- as.formula(euroqol6 ~ itt_treat + age + sexm1 + indepinadl_rand + antiplat_rand + atrialfib_rand + sbprand + dbprand + gcs_score_rand + nihss + sevendaycase + stroketype + R_infarct_size)
...
euroqol_formula <- as.formula(reverse_euroqol_comp ~ itt_treat + age + sexm1 + indepinadl_rand + antiplat_rand + atrialfib_rand + sbprand + dbprand + gcs_score_rand + nihss + sevendaycase + stroketype + R_infarct_size)
...
euroqol_formula <- as.formula(bad_euroqol_comp_bin ~ itt_treat + age + sexm1 + indepinadl_rand + antiplat_rand + atrialfib_rand + sbprand + dbprand + gcs_score_rand + nihss + sevendaycase + stroketype + R_infarct_size)
...
- 何故かtPAが6ヶ月後の予後不良と関連している結果となってしまった
- 一方で、どの解析も同じような結果だった
