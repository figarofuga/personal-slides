...
- ATEの計算をして、各々の(ITE-ATE) ~ (treat - propensity score)でOrthogonalizationで出した係数
- grfの中ではATEとCATEを別個にみるものがある(`test_calibration()`)
  - mean.forest.predictionが群全体のATE: 1なら正確にATEを見つけられている
  - differential.forest.predictionがHeterogeneity: 1ならHeterogneityがある
- 実際の論文でも`grf::test_calibration()`だけを評価しているものが多い
...
- VITAL-AF trialのSub解析
- 高齢者の中で「どの患者で単回のAF screeningが有用か？」を見つける
- T-learnerとRisk scoreを比較
- Risk score毎にScreening betterを評価
- 通常のRisk score研究
[@Shah2025-ie]
...
- 集団全体では、`ca = 1` によりイベントリスクが平均5 percentage points低下する
