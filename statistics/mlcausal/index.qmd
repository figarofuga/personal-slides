...
#   geom_smooth_3d(aes(fill = factor(sex)), alpha = 0.5) + 
#   coord_3d() + 
#   theme_bw()
  

full_toy_data |> 
  dplyr::select(sex, age, lvef, treatment_effect) |> 
  dplyr::mutate(
...
W <- toy_data[, c("age","sex","bmi","hf","bnp","lvef","palpitation")]
...
g_fit <- glm(A ~ age + sex + bmi + hf + bnp + lvef + palpitation, data = toy_data, family = binomial())
...
Q_fit <- glm(Y ~ A + age + sex + bmi + hf + bnp + lvef + palpitation, data = toy_data, family = gaussian())
...
fit_full <- glm(afeqt_os ~ ca + age + sex + bmi + hf + bnp + lvef + palpitation, data = dat_ortho, family = gaussian())
...
fit_y <- glm(
  afeqt_os ~ age + sex + bmi + hf + bnp + lvef + palpitation, 
  data = dat_ortho, 
  family = gaussian()
)
...
fit_a_lpm <- glm(
  ca ~ age + sex + bmi + hf + bnp + lvef + palpitation,
  family = gaussian(),
  data = dat_ortho
)
